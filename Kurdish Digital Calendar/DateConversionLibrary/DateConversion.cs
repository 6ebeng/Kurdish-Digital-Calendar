...
namespace Kurdish_Digital_Calendar.DateConversionLibrary
{
    internal class DateConversion
    {
        public static string ConvertDateBasedOnUserSelection(string selectedText, bool isReverse, string targetDialect, string fromFormat, string toFormat, string targetCalendar, bool isAddSuffix)
        {
            ...
            {
                parsedDate = ExtractSimpleDate(selectedText, fromDateFormat, calendarType);
            } else {
                parsedDate = ExtractComplexDate(selectedText, fromDateFormat, calendarType);
            }
            ...
            formatChoice = Helpers.SelectFormatChoice(toDateFormat);
        ...
        public static DateTime ExtractComplexDate(string dateString, string dateFormat, string targetCalendarType)
        {
            int dayPart = 0, monthPart = 0, yearPart = 0;
            bool isDayParsed = false, isMonthParsed = false, isYearParsed = false;
            Dictionary<string, int> monthNames = new Dictionary<string, int>(StringComparer.OrdinalIgnoreCase);
            ...
            switch (dateFormat)
            {
                case "dddd, dd MMMM, yyyy":
                    var tempParts = dateString.Split(new string[] { ", " }, StringSplitOptions.None);
                    if (tempParts.Length != 3) return DateTime.MinValue; // Invalid format if not split into 3 parts

                    dateString = tempParts[1].Trim(); // Assume format includes day name prefix, which we ignore

                    var tempPartMD = dateString.Split(' ');
                    if (tempPartMD.Length != 2) return DateTime.MinValue; // Invalid format if not split into 2 parts
                ...
                case "dddd, dd/MM/yyyy":
                    ...
                    if (tempParts.Length != 2) return DateTime.MinValue; // Invalid format if not split into 2 parts
        ...
        private static string FormatMonthName(string monthName)
        {
            ...
            if (monthName != null && monthName.Length > 1)
            {
                ...
                return char.ToUpper(monthName[0]) + monthName.Substring(1).ToLower(CultureInfo.InvariantCulture);
            }
            else
            {
        ...
        public static DateTime ExtractSimpleDate(string dateString, string dateFormat, string targetCalendarType)
        {
            int dayPart, monthPart, yearPart;
            string[] dateParts;
            ...
            bool isMonthParsed, isDayParsed, isYearParsed;
            switch (dateFormat)
            {
                case "MM/dd/yyyy":
        ...
        private static string IdentifyDelimiter(string text)
        {
        ...
        public static bool ValidateDateComponents(int day, int month, int year, string calendarType)
        {
            int daysInMonth = 0;
            ...
            switch (calendarType)
            {
                ...
                case "Hijri":
                    ...
                    isLeapYear = new[] { 2, 5, 7, 10, 13, 16, 18, 21, 24, 26, 29 }.Contains(year % 30);
                    daysInMonth = month == 12 ? (isLeapYear ? 30 : 29) : ((month % 2 == 0) ? 29 : 30);
                    return day >= 1 && day <= daysInMonth;

                case "Kurdish (Central)":
                case "Kurdish (Northern)":
                    if (month < 1 || month > 12 || year < 2622) return false; // Kurdish calendar year for Gregorian year 1
        ...
        private static void PopulateMonthNames(Dictionary<string, int> monthNames, string targetCalendarType)
        {
        ...
        private static string CleanSelectedText(string selectedText)
        {
        ...
        private static DateTime ConvertTargetCalendarToGregorian(DateTime date, string calendarType)
        {
            switch (calendarType)
            {
                case "Gregorian":
                ...
                case "Gregorian (Arabic)":
                    return date;
                case "Hijri":
                    return HijriDate.FromHijriToGregorian(date).Date;
                case "Umm al-Qura":
                    return UmmAlQuraDate.FromUmmAlQuraToGregorian(date).Date;
                case "Hijri (English)":
                    return HijriDate.FromHijriToGregorian(date).Date;
                case "Hijri (Arabic)":
                    return HijriDate.FromHijriToGregorian(date).Date;
                ...
                case "Umm al-Qura (Arabic)":
                    return UmmAlQuraDate.FromUmmAlQuraToGregorian(date).Date;
                case "Kurdish (Central)":
                    return KurdishDate.fromKurdishToGregorian(date.Day, date.Month, date.Year);
                case "Kurdish (Northern)":
                    return KurdishDate.fromKurdishToGregorian(date.Day, date.Month, date.Year);
                default:
                    MessageBox.Show("Unsupported calendar type selected.", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                    return date;
            }
        }

        private static string ConvertGregorianToTargetCalendar(DateTime date, int formatChoice, string calendarType, bool isAddSuffix)
        {
            switch (calendarType)
            {
                case "Gregorian (English)":
                    return GregorianDate.FormatGregorian(date, formatChoice, "English", isAddSuffix);
                case "Gregorian (Arabic)":
                    return GregorianDate.FormatGregorian(date, formatChoice, "Arabic", isAddSuffix);
                case "Gregorian (Kurdish Central)":
                    return GregorianDate.FormatGregorian(date, formatChoice, "Kurdish (Central)", isAddSuffix);
                case "Gregorian (Kurdish Northern)":
                    return GregorianDate.FormatGregorian(date, formatChoice, "Kurdish (Northern)", isAddSuffix);
                case "Hijri (English)":
                    return HijriDate.FromGregorianToHijri(date, formatChoice, "English", isAddSuffix);
                case "Hijri (Arabic)":
                    return HijriDate.FromGregorianToHijri(date, formatChoice, "Arabic", isAddSuffix);
                case "Hijri (Kurdish Central)":
                    return HijriDate.FromGregorianToHijri(date, formatChoice, "Kurdish (Central)", isAddSuffix);
                case "Hijri (Kurdish Northern)":
                    return HijriDate.FromGregorianToHijri(date, formatChoice, "Kurdish (Northern)", isAddSuffix);
                case "Umm al-Qura (English)":
                    return UmmAlQuraDate.FromGregorianToUmmAlQura(date, formatChoice, "English", isAddSuffix);
                case "Umm al-Qura (Arabic)":
                    return UmmAlQuraDate.FromGregorianToUmmAlQura(date, formatChoice, "Arabic", isAddSuffix);
                case "Umm al-Qura (Kurdish Central)":
                    return UmmAlQuraDate.FromGregorianToUmmAlQura(date, formatChoice, "Kurdish (Central)", isAddSuffix);
                case "Umm al-Qura (Kurdish Northern)":
                    return UmmAlQuraDate.FromGregorianToUmmAlQura(date, formatChoice, "Kurdish (Northern)", isAddSuffix);
                case "Kurdish (Central)":
                    return KurdishDate.fromGregorianToKurdish(date, formatChoice, calendarType, isAddSuffix);
                case "Kurdish (Northern)":
                    return KurdishDate.fromGregorianToKurdish(date, formatChoice, calendarType, isAddSuffix);
                default:
                    MessageBox.Show("Unsupported calendar type selected.", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
