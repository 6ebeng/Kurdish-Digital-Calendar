...
namespace KDC_Viso
{
    ...
    public class Ribbon : IRibbonExtensibility
    {
        ...
        private void populateInsertDate()
        {
            ...
            if (activeWindow.Selection.Count > 0)
            {
                foreach (Visio.Shape shape in activeWindow.Selection)
                {
                    ...
                    shape.Text = kDCService.Kurdish(formatChoice, _selectedDialect, _isAddSuffix);
                }
            }
            else
            {
        ...
        public void SplitButton2_Click(IRibbonControl control)
        {
            ...
            Visio.Selection selection = Globals.ThisAddIn.Application.ActiveWindow.Selection;
            ...
            foreach (Visio.Shape shape in selection)
            {
                ...
                if (!string.IsNullOrEmpty(shape.Text))
                {
                    ...
                    // Try to parse the shape's text as a DateTime
                    if (DateTime.TryParse(selectedText, out dateValue))
                    {
                        // If successful, format the date to a short date string
                        selectedText = dateValue.ToString("d"); // 'd' format returns the short date pattern
                    }
                    ...
                    if (result == selectedText || string.IsNullOrEmpty(result))
                    {
                        // If the result is the same as the input or empty, indicate an error visually
                        // Visio API does not directly support background color change like Excel; consider using shape data graphics or a different visual indicator
                    }
                    else
                    {
                        // Update the shape's text with the result
                        shape.Text = result;

                        // Visually reset any error indication you might have added above
                    }
                }
            }
        }
